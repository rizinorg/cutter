...
void HexWidget::wheelEvent(QWheelEvent *event)
{
    ...
    if (dy != 0) {
        ...
        if (cursor.addr >= startAddr && cursor.addr <= (startAddr + bytesPerScreen())) {
            /* Don't enable cursor blinking if selection != 0 */
            if (!selection.isValid)
                cursorEnabled = true;
            updateCursorMeta();
        } else {
...
void HexWidget::drawItemArea(QPainter &painter)
{
    ...
    if (selection.intersects(startAddr, startAddr + bytesPerScreen())) {
        ...
        int selBeginOffset2 = (selBeginOffset + (itemRowByteLen() - 1)) & ~(itemRowByteLen() - 1);
        int selEndOffset2 = selEndOffset & ~(itemRowByteLen() - 1);
        if (selBeginOffset2 <= selEndOffset2) {
            /* Fill top piece if exists */
            if (selBeginOffset != selBeginOffset2) {
                QRect rect(itemRectangle(selBeginOffset));
                rect.setRight(itemArea.right());
                painter.fillRect(rect, Qt::blue);
            }
            /* Fill bottom piece if exists */
            //if (selEndOffset != selEndOffset2) {
                QRect rect(itemRectangle(selEndOffset));
                rect.setLeft(itemArea.left());
                painter.fillRect(rect, Qt::blue);
            //}
            --selEndOffset2; // need to properly fill main mody
        } else {
            selBeginOffset2 = selBeginOffset;
            selEndOffset2 = selEndOffset;
        }
        /* Fill main body */
        if (selBeginOffset2 <= selEndOffset2) {
            QRect rect(itemRectangle(selBeginOffset2));
            rect.setBottomRight(itemRectangle(selEndOffset2).bottomRight());
    ...
    for (int i = 0; i < rowCount; ++i) {
        ...
        for (int j = 0; j < itemColumns; ++j) {
            for (int k = 0; k < itemGroupSize; ++k, itemOffset += itemByteLen) {
                ...
                if (selection.isValid && itemOffset >= selBeginOffset && itemOffset <= selEndOffset)
...
void HexWidget::drawAsciiArea(QPainter &painter)
{
    ...
    /* FIXME: Copy/paste from drawItemArea (itemArea -> asciiArea; itemRectangle -> asciiRectangle)*/
    int selBeginOffset = -1;
    int selEndOffset = -1;
    if (selection.intersects(startAddr, startAddr + bytesPerScreen())) {
        selBeginOffset = std::max(selection.begin, startAddr) - startAddr;
        ...
        if (selBeginOffset2 <= selEndOffset2) {
            /* Fill top piece if exists */
            if (selBeginOffset != selBeginOffset2) {
                QRect rect(asciiRectangle(selBeginOffset));
                rect.setRight(asciiArea.right());
                painter.fillRect(rect, Qt::blue);
            }
            /* Fill bottom piece if exists */
            //if (selEndOffset != selEndOffset2) {
                QRect rect(asciiRectangle(selEndOffset));
        ...
        /* Fill main body */
        if (selBeginOffset2 <= selEndOffset2) {
            QRect rect(asciiRectangle(selBeginOffset2));
            rect.setBottomRight(asciiRectangle(selEndOffset2).bottomRight());
            painter.fillRect(rect, Qt::blue); // FIXME: honor colortheme
        }
    }
    ...
    for (int line = 0; line < rowCount; ++line, charRect.translate(0, rowHeight)) {
        ...
        for (int j = 0; j < itemRowByteLen(); ++j, ++byteId) {
            ...
            if (selection.isValid && byteId >= selBeginOffset && byteId <= selEndOffset)
